      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3 text-xs font-semibold">
            <span className="inline-flex items-center px-3 py-1 rounded-full bg-[#F5F8F6] text-[#075C49] border border-[#DCE8E3] tracking-wide">
              {article.category}
            </span>
            ...
            <span className="text-neutral-300">•</span>
            <div className="flex items-center gap-1.5 text-neutral-500 font-medium">
              <Calendar className="w-3.5 h-3.5 text-[#075C49]" />
          ...
          <div className="pt-4 flex items-center justify-between gap-4 border-t border-neutral-100">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#F5F8F6] border border-[#DCE8E3] text-[#075C49] flex items-center justify-center font-bold text-sm flex-shrink-0">
                <User className="w-5 h-5 stroke-[2]" />
              </div>
              <div>
                <div className="font-bold text-sm text-neutral-900">
                  {article.authorSlug ? (
                    <Link to={`/professionals/${article.authorSlug}`} className="hover:text-[#075C49] transition-colors">
                      {article.author}
                    </Link>
                  ) : (
                    article.author
            ...
            <Link
              to={ROUTES.RESOURCES}
              className="inline-flex items-center gap-1.5 text-xs font-bold text-[#075C49] hover:text-[#054839] px-3 py-1.5 rounded-xl hover:bg-[#F5F8F6] transition-colors"
            >
