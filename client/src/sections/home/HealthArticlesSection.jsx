    ...
    >
      <Container>
        ...
        <header className="text-center max-w-2xl mx-auto mb-8 sm:mb-12 space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#F1F8F4] border border-[#DCEBE4] shadow-2xs">
            <span className="w-1.5 h-1.5 rounded-full bg-[#98CF2D]" aria-hidden="true" />
            <span className="text-[10px] sm:text-[11px] font-bold tracking-[0.2em] text-[#075B43] uppercase">
              HEALTH &amp; WELLNESS
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 items-stretch max-w-6xl mx-auto">
          {HEALTH_ARTICLES.map((article) => (
            <article
              key={article.id}
              className="group relative flex flex-col justify-between bg-white rounded-2xl border border-[#DCEBE4] hover:border-[#B7D9CA] shadow-soft-xs hover:shadow-soft-sm transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="p-4 sm:p-5 flex flex-col justify-between flex-grow space-y-1.5 text-left">
                <span className="text-[10px] sm:text-[11px] font-bold text-[#0D7657] tracking-widest uppercase">
                  {article.category}
                </span>
                ...
                <div className="flex items-center justify-between gap-3 pt-0.5">
                  ...
                  <ArrowRight
                    className="w-4 h-4 text-neutral-400 group-hover:text-[#075B43] transition-all duration-200 group-hover:translate-x-1 flex-shrink-0"
                    aria-hidden="true"
                  />
                </div>
              </div>
        ...
        <div className="mt-8 sm:mt-10 text-center">
          <Link
            to={ROUTES.RESOURCES}
            className="inline-flex items-center gap-1.5 font-bold text-xs sm:text-sm text-[#075B43] hover:text-[#064C38] transition-colors group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm py-1"
          >
