    ...
    >
      <Container>
        ...
        <header className="text-center max-w-2xl mx-auto mb-8 sm:mb-12 space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-[#F5F8F6] border border-[#DCE8E3] shadow-2xs">
            <span className="w-1.5 h-1.5 rounded-full bg-[#8FD21F]" aria-hidden="true" />
            <span className="text-[10px] sm:text-[11px] font-bold tracking-[0.2em] text-[#075C49] uppercase">
              HEALTH &amp; WELLNESS
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 items-stretch max-w-6xl mx-auto">
          {HEALTH_ARTICLES.map((article) => (
            <article
              key={article.id}
              className="group relative flex flex-col justify-between bg-white rounded-2xl border border-[#DCE8E3] hover:border-[#B7D9CA] shadow-soft-xs hover:shadow-soft-sm transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="p-4 sm:p-5 flex flex-col justify-between flex-grow space-y-1.5 text-left">
                <span className="text-[10px] sm:text-[11px] font-bold text-[#0C6F58] tracking-widest uppercase">
                  {article.category}
                </span>

                <div className="flex items-center justify-between gap-3 pt-0.5">
                  <h3 className="font-heading text-sm sm:text-base font-bold text-neutral-900 group-hover:text-[#075C49] transition-colors leading-snug">
                    <Link
                  ...
                  <ArrowRight
                    className="w-4 h-4 text-neutral-400 group-hover:text-[#075C49] transition-all duration-200 group-hover:translate-x-1 flex-shrink-0"
                    aria-hidden="true"
                  />
                </div>
              </div>
        ...
        <div className="mt-8 sm:mt-10 text-center">
          <Link
            to={ROUTES.RESOURCES}
            className="inline-flex items-center gap-1.5 font-bold text-xs sm:text-sm text-[#075C49] hover:text-[#054839] transition-colors group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-sm py-1"
          >
