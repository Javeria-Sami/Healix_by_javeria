  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between text-xs text-text-muted border-t border-border/40 pt-4">
        <span>
          Showing <strong className="text-text-primary font-semibold">{totalResults}</strong> {totalResults === 1 ? 'question' : 'questions'}
          {selectedCategory !== 'All Questions' && ` in "${selectedCategory}"`}
          {searchQuery && ` matching "${searchQuery}"`}
        </span>

        {(selectedCategory !== 'All Questions' || searchQuery) && (
          <button
            type="button"
            onClick={() => {
              onSelectCategory('All Questions');
              onSearchChange('');
            }}
